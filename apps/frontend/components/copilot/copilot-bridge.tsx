...
export type DockTool = {
   ...
   *
   * Quy ước đó được ÉP BẰNG KIỂU, không chỉ nằm trong comment này: nhánh object
   * là `{ message: string } & Record<string, unknown>` nên một tool trả object
   * mà quên `message` sẽ hỏng lúc compile, chứ không âm thầm rò JSON thô ra cho
   * user lúc chạy — đúng cái bẫy mà lớp card này sinh ra để chặn.
   */
  handler: (
    args: Record<string, unknown>,
  ) => Promise<string | ({ message: string } & Record<string, unknown>)>;
};
